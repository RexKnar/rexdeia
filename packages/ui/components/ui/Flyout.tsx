...
const Flyout: FC<FlyoutProps> = ({ isOpen, onClose, children, widthClass }) => {

  return (
    <>
      {isOpen && (
        <div className="fixed inset-0 backdrop-filter backdrop-blur-sm bg-black bg-opacity-20">
          <div className={`fixed bg-white p-3 rounded-l-md top-0 right-0 h-full transform translate-x-0 transition-transform duration-300 ease-in-out ${widthClass}`}>
            {children}
