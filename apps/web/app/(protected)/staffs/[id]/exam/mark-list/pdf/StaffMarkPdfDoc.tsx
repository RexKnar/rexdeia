...
const StudentMarksPDFGenerator: React.FC<StudentMarksPDFGeneratorProps> = ({
  ...
  const generatePDF = () => {
    ...
    let totalCell = subjectList.length > 1 ? 4 : 3;
    let complexHeaders =
      headingList?.length > 0
        ? [
            [
              ...
              subjectList.length > 1 ? { content: 'Total' } : null,
              subjectList.length > 1 ? { content: 'Rank' } : null,
            ],
          ]
        : [];
    const subHeading =
      subjectList.length > 1
        ...
        : [
            {
              content: ` ${examDetails?.name} Mark List || ${classDetails?.name} - ${sectionDetails?.name} `,
              colSpan: 3,
            },
          ];
