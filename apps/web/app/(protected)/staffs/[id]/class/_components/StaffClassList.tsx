...
import { StaffClassWidget } from './StaffClassWidget';

export function StaffClassList() {
  const { data: session } = useSession();
  const router = useRouter();
  const [staffId, setStaffId] = useState('');
  const { data: classList, isLoading: isClassListLoading } =
    useGetSubjectByStaffIdQuery(staffId, {
      enabled: !!staffId,
    });
  useEffect(() => {
    ...
    <section className="p-3">
      {classList?.length > 0 && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {classList?.map((widgetData) => (
            <StaffClassWidget classDetails={widgetData} key={widgetData.id} />
          ))}
        </div>
      )}
    </section>
