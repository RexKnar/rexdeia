...
import { getBranchById, updateBranchById } from './service';
 ...
 *       description: Get the details of an existing branch.
 *       parameters:
 *         - name: id
 *           in: path
 *           required: true
...
export async function GET(request: Request, { params: { id } }) {
  ...
  try {
    const branchDetails = await getBranchById(id);

    return new NextResponse(JSON.stringify(branchDetails), {
      status: StatusCodes.OK,
    });
  } catch (e) {
    captureException(e);
