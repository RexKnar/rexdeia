 ...
 *     post:
 *       summary: Add Students to section
 *       description: Add Students to existing section
 *       parameters:
 *         - name: id
 *           in: path
 *           required: true
 ...
 *       responses:
 *         '200':
 *           description: Students details added successfully.
 *           content:
 *             application/json:
 *               schema:
...
export async function POST(request: NextRequest, { params: { id } }) {
  ...
  try {
    const section = await addStudentsToSection(id, payload['studentIds']);
 ...
 *       description: Get All students in a section
 *       parameters:
 *         - name: id
 *           in: path
 *           required: true
 ...
 *       responses:
 *         '200':
 *           description: students details are fetched successfully.
 *           content:
 *             application/json:
 *               schema:
 *                 # Define the schema of your class object here
 ...
 * /api/section/{id}/students:
 *     delete:
 *       summary: Remove students from section
 *       description: Remove students from existing section
 *       parameters:
 *         - name: id
 *           in: path
 *           required: true
 ...
 *       responses:
 *         '200':
 *           description: student details removed successfully.
 *           content:
 *             application/json:
 *               schema:
...
export async function DELETE(request: NextRequest, { params: { id } }) {
  ...
  const payload: { studentIds: string[] } = await request.json();

  try {
    const section = await removeStudentsFromSection(id, payload.studentIds);
    return new NextResponse(JSON.stringify(section), {
      status: StatusCodes.OK,
    });
  } catch (e) {
