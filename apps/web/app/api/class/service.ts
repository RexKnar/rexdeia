...
export async function getAllStudentByClassIdForAssigning(id: string, includeArchived = false) {
  const where: any = {
    classId: id,
    section: null,
  };
  ...
  const studentResponse = studentList.map((mapping) => ({
    ...mapping.student,
    isArchived: !mapping.isCurrent,
  }));
